...
```{r}
employment_by_occup_group <- readRDS("final_exam/Employment_by_occup_group.rds")
```
...
```{r}
airport_tran <-read_excel("final_exam/tuik_transportation_by_airport.xlsx")
```

Then, it is saved it in **RData** format.

```{r}
save(airport_tran, file = "final_exam/airport_tran.RData")
```

```{r}
load(file = "final_exam/airport_tran.RData")
```
